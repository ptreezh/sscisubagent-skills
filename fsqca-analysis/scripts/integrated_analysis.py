...
class IntegratedFSCAnalyzer:
    ...
    def __init__(self, skill_root: str):
        ...
        self.truth_table_builder = FuzzyTruthTableBuilder()
        self.minimizer = FSCMinimizer()
    ...
    def execute_quantitative_analysis(self,
        ...
        quantitative_results = {
            ...
            'technical_details': {
                ...
                'n_positive_cases': len(truth_table[truth_table['outcome'] >= 0.5]),
    ...
    def _create_variable_calibration_plan(self,
        ...
        if var_info['type'] == 'continuous':
            if var_info['unique_values'] > 20:
                ...
                plan['recommended_method'] = 'interpolation'
                plan['reason'] = '连续变量但取值有限，适合插值校准'
        elif var_info['type'] == 'discrete_numeric':
            plan['recommended_method'] = 'threshold'
            plan['reason'] = '离散数值变量，适合阈值校准'
        else:
            plan['recommended_method'] = 'indirect'
            plan['reason'] = '分类变量，适合间接校准'
            ...
            if variable in data.columns:
                ...
                if method == 'direct':
                    ...
                    calibrated_series = self.calibrator.calibrate_variable(
                        ...
                        thresholds=(data[variable].quantile(0.95), 
                                  data[variable].quantile(0.50), 
                                  data[variable].quantile(0.05))
                    )
                elif method == 'threshold':
                    calibrated_series = self.calibrator.calibrate_variable(
                        data[variable], method='threshold',
                        thresholds=(data[variable].quantile(0.8), 
                                  data[variable].quantile(0.2))
                    )
                elif method == 'interpolation':
                    calibrated_series = self.calibrator.calibrate_variable(
                        data[variable], method='interpolation'
                    )
    ...
    def _calculate_analysis_quality(self,
        ...
        if solutions:
            # 计算所有解的平均质量，避免空列表问题
            try:
                total_coverage = sum(sol.coverage for sol in solutions if not np.isnan(sol.coverage))
                total_consistency = sum(sol.consistency for sol in solutions if not np.isnan(sol.consistency))
                valid_solutions_count = len([sol for sol in solutions if not np.isnan(sol.coverage) and not np.isnan(sol.consistency)])

                if valid_solutions_count > 0:
                    avg_coverage = total_coverage / valid_solutions_count
                    avg_consistency = total_consistency / valid_solutions_count
                else:
                    avg_coverage = 0.0
                    avg_consistency = 0.0
            except:
            ...
            # 找到最佳解，过滤掉NaN值
            valid_solutions = [sol for sol in solutions if not np.isnan(sol.coverage) and not np.isnan(sol.consistency)]
            if valid_solutions:
                best_solution = max(valid_solutions, key=lambda x: x.coverage * x.consistency)
                quality_metrics['best_solution'] = {
                    'coverage': best_solution.coverage,
                    'consistency': best_solution.consistency,
    ...
    def _calculate_overall_quality(self, quality_metrics: Dict[str, Any]) -> float:
        ...
        if 'solution_quality' in quality_metrics:
            avg_coverage = quality_metrics['solution_quality'].get('avg_coverage', 0.0)
            avg_consistency = quality_metrics['solution_quality'].get('avg_consistency', 0.0)

            # 处理NaN值
            if np.isnan(avg_coverage):
                avg_coverage = 0.0
            if np.isnan(avg_consistency):
                avg_consistency = 0.0

            solution_score = (avg_coverage + avg_consistency) / 2
        else:
            solution_score = 0.5
    ...
    def _create_interpretation_guidance(self,
        ...
        # 识别关键发现
        if quantitative_results['solutions']:
    ...
    def generate_analysis_report(self, output_file: str = None) -> str:
        ...
        # 报告标题
        report_sections.append("# fsQCA集成分析报告\n")
        ...
        if self.analysis_state['quantitative_results']:
            ...
            if 'best_solution' in quality:
                report_sections.append("### 最优解\n")
                best = quality['best_solution']
                report_sections.append(f"- 覆盖度: {best['coverage']:.3f}\n")
                report_sections.append(f"- 一致性: {best['consistency']:.3f}\n")
                report_sections.append(f"- 复杂度: {best['complexity']}\n")
...
def main():
    """示例用法"""
    # 设置技能根目录
    skill_root = "D:/stigmergy-CLI-Multi-Agents/sscisubagent-skills/fsqca-analysis"
    ...
    sample_data = pd.DataFrame({
        'case_id': range(1, 21),
        'A': np.random.uniform(0, 1, 20),  # 条件A，模糊集隶属度
        'B': np.random.uniform(0, 1, 20),  # 条件B，模糊集隶属度
        'C': np.random.uniform(0, 1, 20),  # 条件C，模糊集隶属度
        'Y': np.random.uniform(0, 1, 20)   # 结果变量，模糊集隶属度
    })
    ...
    # 生成报告
    report = analyzer.generate_analysis_report("fsqca_analysis_report.md")
