...
- 返回错误: `ModuleNotFoundError: No module named 'statsmodels'`

**问题分析**:
- 该技能需要额外的统计分析包
- 这些包未包含在核心依赖中，需要单独安装
- 这是预期行为，因为这些包比较专业且体积较大
...
- 返回错误: `ModuleNotFoundError: No module named 'factor_analyzer'`
...
### 依赖管理优势
1. 使用 uv 提供更快的依赖解析和安装速度
2. 支持国内镜像源（如清华源）加速下载
3. 可选依赖允许用户按需安装，减少不必要的包
4. 配置了缓存策略以提高重复安装效率
...
psychometrics = ["factor_analyzer", "pingouin"]
all = ["ssci-skills[basic,statistics,psychometrics]"]
```
